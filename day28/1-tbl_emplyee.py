...
def select_emp():
    ...
    sql = "SELECT * FROM employee ORDER BY salary DESC" # 검색 SQL 언어 (ASC = 오름차순, DESC = 내림차순)
    cur.execute(sql)                                    # sql 실행
    # 자료 가져오기
    rs = cur.fetchall()                                 # rs : result set
    # print(rs)
    for i in rs:
        print(i)
    conn.close()                                        # 연결 종료
...
def select_one():
    ...
    cur.execute(sql)
    rs = cur.fetchone()                                 # 1개 반환 - 자료 변동 없으므로 commit 사용 안함
    print(rs)
    conn.close()
...
# select_one()
# insert_emp()                                          # 재실행시 emp_id는 중복 불가이므로 에러남
# update_emp()
delete_emp()
select_emp()
